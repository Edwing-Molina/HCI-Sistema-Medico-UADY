...
Deliverables: [Quality Attributes v1](/2nd%20Delivery/Usability_Attributes.md)
Metrics: [Accessibility](/2nd%20Delivery/Metrics/AccesibilityMetrics.md) · [Operability](/2nd%20Delivery/Metrics/Operability.md) · [Satisfaction](/2nd%20Delivery/Metrics/SatisfactionMetrics.md) · [UserErrorProtection](/2nd%20Delivery/Metrics/UserErrorProtection.md)
...
**Definitive Conclusion:** **Age is a correlation, not the cause.** The real variable is **digital competence and familiarity with mobile interfaces**.

### 2.3 Shared Strengths (Both Groups)

**1. Trust in Institutional Branding:** Both groups showed high trust in UADY + Microsoft SSO. This compensates for friction in other areas, meaning investment in branding is an investment in UX.

**2. Acceptance of Business Rules:** Both groups immediately understood why past-date appointments cannot be scheduled and did not repeatedly click disabled buttons. Business logic is intuitive; no changes required.

**3. Patience with Loading States:** Both groups recorded 0% repeated clicks while loading, waiting patiently throughout. Loading states are well designed.

### 2.4 Common Problems (Both Groups)

**1. Login = Critical Entry Barrier:** 25% of each group required assistance. Root cause: virtual keyboard combined with complex passwords. This reduces adoption rate.

**2. Post-Booking Friction (50–80% Abandonment):** Users in both groups got lost searching for appointment details after booking. Root cause: confusing Information Architecture in the appointments section.

**3. Lack of Clarity in Transaction States:** Both groups did not immediately understand the meaning of "Requested" vs "Scheduled" and asked validation questions ("Was it saved?"). Labels are not explicit enough.

### 2.5 Critical Differences Between Groups

**Difference 1 — Attitude Toward Exploration:** Group A has some users who actively explore while others avoid it. Group B largely requests confirmation before exploring, meaning Group B requires a more guided interface or tutorial.

**Difference 2 — Error Tolerance:** Group A shows the full range from SUS 100 (0 errors) to SUS 40 (many errors). Group B has a more consistent distribution with 3–22 errors. Group B needs stronger error protection: larger buttons and more spacing.

**Difference 3 — Motor Precision:** Group A reported no motor issues. Grazielita (Group B) experienced 5 accidental clicks, likely due to post-shift fatigue and reduced familiarity with touch screens. Button target sizes need to increase.
...
All users confirmed they **would use the app frequently**. Reason: severe dissatisfaction with the current system (long waits, unanswered phone calls). Everyone perceived the UADY + Microsoft branding as **secure and professional**. However, they acknowledged **there are small details to improve**. Marginal SUS scores (70–77) indicate that these "small details" are actually **significant operational friction**.
...
**Problem:** The virtual keyboard causes multiple typos, frustration, and abandonment. Users must type email + complex password, which is especially problematic for users with low motor precision.
...
**Benefits:** Eliminates 100% of virtual keyboard problems, faster (1 tap vs 50+ taps), more secure, and accessible for users of all ages.

**Implementation:** Integrate with device biometric APIs (iOS Face ID, Android Biometric API). Use Microsoft Authenticator as an alternative. Maintain email/password option as fallback and promote biometrics as the preferred method.
...
**Problem:** The user successfully completes the booking but does not know where to find the appointment. 50–80% abandonment when trying to validate the appointment, with uncertainty expressed as "Was it saved?" and "Where is it?"

**Proposed Solution: Immediate Visible Ticket**

```
BEFORE (Current):
Step 1–4: [Book appointment]
[Screen returns to HOME]
"Where is my appointment?" → User gets lost
...
**Benefits:** Immediate visual confirmation of success, key information visible at a glance, the user can take a screenshot for reference, and reduces uncertainty.

**Implementation:** Create a modal/full-screen confirmation after appointment creation endpoint. Show key data (doctor, specialty, date, time, campus, reference number). Include "View Details" and "Back to Home" buttons.
...
**Problem:** Users reported difficulty reading appointment details. 75% of Group A wears glasses and many requested larger font sizes. Cards likely have low contrast or small text, especially affecting Group B.
...
**Specifications:** Font minimum 18–20px (from 16px), contrast ratio 4.5:1 for normal text, increased padding between elements, emoji-based visual hierarchy, color + icon state coding (green, yellow, red), and simplified main view (5–6 key data points with "View Details" for the rest).
...
**Problem:** After booking, users try to find their appointment but encounter confusing Information Architecture. Active tabs are not clearly distinguishable, leading to 50–80% abandonment while searching for details.
...
Active tab: colored background, white text. Inactive tab: light gray background, dark text. Animated underline on active tab and emojis for quick identification.
...
**Problem:** Grazielita (Group B) made 5 accidental clicks, likely due to post-shift fatigue. Hit targets are too small (< 44×44 px).

**Proposed Solution:** Increase primary buttons to 56×56 px (from 44–48 px), spacing between buttons +8px, and increase internal padding. The recommendation is 48×48 px minimum for accessibility compliance.
...
**Problem:** 50% of Group B expressed fear of exploring. Some requested confirmation even before obvious buttons. Low-competence users lack initial guidance.

**Proposed Solution: Optional First-Run Tutorial** with welcome screen, step-by-step how-to-book walkthrough, how to view appointments, and how to cancel. Shown only on first run with the option always available in a Help section.
...
**Problem:** When booking in a past month is not allowed, the calendar is only grayed out without explanation. Alejandro suggested an explicit banner explaining the restriction.
...
**P2.1: Haptic Feedback** — Slight vibration when tapping buttons or selecting dropdowns, especially useful for Group B. (+1 SUS point)

**P2.2: Option to Increase Font Size Globally** — Settings > Font Size (Normal / Large / Extra Large), since 75% reported visual issues. (+2 SUS points)

**P2.3: Dark Mode (Optional)** — Toggle in Settings for users with visual difficulties. (+1 SUS point)
...
**Conclusion:** With implementation of P0 (3 critical changes), both groups **reach or exceed the SUS threshold of 80**.

---

## 5. Compliance with Usability Attributes
...
The **UADY Medical Appointment System partially complies with the defined usability attributes**, achieving **75.9% overall compliance** — 4.1% below the 80% target — as measured by scenario traceability for Accessibility, Learnability, and Error Protection, and by direct SUS score for Satisfaction.
...
**Attributes below target:**
- Satisfaction (SUS 73.75 / 100) — both groups fall in the Marginal zone; Group A (77.5) is near the acceptable boundary while Group B (70.0) sits at the lower end, with login friction and post-booking uncertainty identified as the primary drivers of dissatisfaction
- Accessibility (62.5%) — barriers persist for users with visual difficulty and reduced motor precision

**The three changes that would most directly close the compliance gap, ordered by projected impact:**

1. Post-booking confirmation screen (P0.2) — eliminates the main source of post-booking uncertainty that suppresses the SUS score, and resolves LRN-SAT simultaneously (+9–12 SUS points projected)
2. Appointment card readability redesign (P0.3) — resolves ACC-03, the primary Accessibility failure (+12.5% in Accessibility)
3. Increased button target size (P1.2) — resolves ACC-01 partial status, reducing accidental taps for low motor-precision users

All three are low-to-medium effort frontend changes. Together, they would bring the global compliance score to approximately 85.5%, making the system genuinely accessible for the full spectrum of digital competence present in the target population.
...
**Group A (Manual Workers/Younger):** 4 users, ages 35–45. Maintenance, cleaning, and administrative staff. Digital competence: variable (SUS range 40–100).

**Group B (Older Adults):** 4 users, ages 45–55. Maintenance, cleaning, and administrative staff. Digital competence: consistently low–moderate (SUS range 68–72).

### B. Measurement Instruments

- System Usability Scale (SUS) — 10 questions
- Task metrics (clicks, errors, completion rate)
- Qualitative observation (moderator notes)
- Semi-structured post-test interview
...
- `OlderDemographicGroupAnalysis.md` — Detailed Group B analysis
- `TestProtocol.md` — Complete testing protocol
- Traceability Matrices — Accessibility, Learnability, Error Protection, Satisfaction